...
def list_drives():
    ...
    print("=" * 100)
    print(f"{'Device':<12} {'OSD':<6} {'Serial':<20} {'Model':<30} {'Enclosure Bay':<15}")
    print("=" * 100)
    
    # Show drives with their info
    for serial, drive in sorted(data['drives'].items(), key=lambda x: x[1].get('current_device', '')):
    ...
    if data['enclosures']:
        ...
        for host, enc in data['enclosures'].items():
            print(f"  Host {host}: {enc['name']} ({enc['device']}) - {len(enc['slots'])} mapped slots")
    else:
...
def main():
    ...
    if device_arg.lower().startswith('osd.') or device_arg.lower().startswith('osd'):
        osd_id = device_arg.replace('OSD.', '').replace('osd.', '').replace('OSD', '').replace('osd', '')
